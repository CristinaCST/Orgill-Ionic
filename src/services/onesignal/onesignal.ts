...
export class OneSignalService {
  ...
  public init(): void {
    ...
    this.oneSignal.handleNotificationReceived().subscribe(data => {
      if (data && data.payload) {
        // Validate the structure of the notification
  ...
  public setPermissions(): void {
    ...
    ).subscribe(([notification]: DefaultPopoverResult[]) => {
      // For each modal we need a matching parameter
      ...
      this.navigationSubscription = this.navigatorService.lastPage.subscribe((pageName: string) => {
        if (pageName === 'Login') {
          ...
          this.oneSignal.getPermissionSubscriptionState().then(state => {
            if (state.permissionStatus.state === androidPermissionState.DENIED) {
              // state == 2 means it's declined, 1 is Authorized.
  ...
  public androidSubscriptionSwitchOn(): void {
    this.oneSignal
      ...
      .then((state: OSPermissionSubscriptionState) => this.setSubscriptionSwitch(state, true));
  }
  ...
  public androidSubscriptionSwitchOff(): void {
    this.oneSignal
      .getPermissionSubscriptionState()
      .then((state: OSPermissionSubscriptionState) => this.setSubscriptionSwitch(state, false));
  }
  ...
  private iOSPermissionsSetup(): Promise<boolean> {
    if (!this.navigationSubscription) {
      this.navigationSubscription = this.navigatorService.lastPage.subscribe(pageName => {
        if (pageName === 'Login') {
          // Check if permission is granted, if not, we show a modal
          this.oneSignal.getPermissionSubscriptionState().then(state => {
            if (state.permissionStatus.status === iOSPermissionState.DENIED) {
              // status == 1 on iOS means Denied
              //   this.showPermissionsReminder();
            } else if (
              state.permissionStatus.status === iOSPermissionState.NOT_DETERMINED &&
              this.modalState.initial < this.modalState.navCount
            ) {
              // status == 0 on iOS means not asked.
              //   this.setPermissions();
            }
          });
        }
    ...
    return this.badge
      .isSupported()
      .then(isSupported => {
        if (isSupported && this.pushNotificationPermission) {
          // .hasPermission() check prompts notification permission so we don't want to do it always
      ...
      .then((requestResult: boolean) => {
        return new Promise(resolve => {
          const askedAndDeclined: string = LocalStorageHelper.getFromLocalStorage(
            Constants.ONE_SIGNAL_IOS_PERMISSION_DECLINED
          );
  ...
  public showPermissionsReminder(): Promise<{}> {
  ...
  private testOneSignal(): void {
     ...
     */
    try {
      const fakeReceivedNotification: OSNotificationPayload = {
        additionalData: { SKU: '0011049' }
      } as OSNotificationPayload;
  ...
  private goToHotDeal(data: OSNotificationPayload): void {
    ...
    if (data) {
      if (sku) {
        ...
        // If there is data but the package does not respect the hot deal structure
        this.debugLog('ONESIGNAL package is not a hot deal but it exists...' + JSON.stringify(data));
      }
    } else {
      // If the package is empty just open the app?
  ...
  private updatePermissions(): void {
    this.pushNotificationPermission =
      LocalStorageHelper.getFromLocalStorage(this.pushNotificationPermissionID) === 'true';
  ...
  private askForPermission(
  ...
  ): Observable<DefaultPopoverResult> {
    ...
    return this.popoversService.show(popoverContent, false, {
      enableBackdropDismiss: false
    }) as Observable<DefaultPopoverResult>;
  }
